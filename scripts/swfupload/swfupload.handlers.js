$(function () {
    ...
    $.fn.InitSWFUpload = function (p) {
        ...
        p.sendurl += "&channel_id=" + p.channelid;
    ...
    try {
        switch (errorCode) {
            case SWFUpload.errorCode_QUEUE_LIMIT_EXCEEDED:
                $.dialog.alert("你选择的文件太多！");
                break;
            case SWFUpload.QUEUE_ERROR.ZERO_BYTE_FILE:
                $.dialog.alert(file.name + "文件太小！");
                break;
            case SWFUpload.QUEUE_ERROR.FILE_EXCEEDS_SIZE_LIMIT:
                $.dialog.alert(file.name + "文件太大！");
                break;
            case SWFUpload.QUEUE_ERROR.INVALID_FILETYPE:
                $.dialog.alert(file.name + "文件类型出错！");
                break;
            default:
                if (file !== null) {
                    $.dialog.alert("出现未知错误！");
...
function uploadProgress(file, bytesLoaded) {
    try {
        ...
        progressObj.find(".txt-bar").html(percent + "%");
    ...
    try {
        switch (errorCode) {
            ...
            case SWFUpload.UPLOAD_ERROR.UPLOAD_LIMIT_EXCEEDED:
                $.dialog.alert(message + "SWFUpload.UPLOAD_ERROR.UPLOAD_LIMIT_EXCEEDED");
                break;
            default:
                $.dialog.alert(message + "未知！");
                break;
        }
    } catch (ex3) {
        this.debug(ex3);
    ...
    try {
        ...
        if (jsonstr.status == '1') {
            ...
            if (this.customSettings.button_action == SWFUpload.BUTTON_ACTION.SELECT_FILE) {
                ...
                $(this.customSettings.view_img).attr("src", jsonstr.path);
            } else {
                addImage($(this.customSettings.upload_target), jsonstr.path, jsonstr.thumb, file.name);
            }
            var progressObj = $(this.customSettings.upload_target).children(".upload-progress");
            progressObj.children(".txt").html("已完成：" + file.name);
        }
    } catch (ex) {
        this.debug(ex);
    }
...
function createHtmlProgress(swfuInstance) {
    ...
    var progressText = $('<span class="txt">正在上传 <b class="txt-bar"></b>...</span>').appendTo(fileProgressObj);
