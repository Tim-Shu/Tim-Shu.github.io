<%@ Page Language="C#" AutoEventWireup="true" CodeBehind="advert_group.aspx.cs" Inherits="QJcms.Web.admin.plugins.advert_group" %>
...
<div id="floatHead" class="toolbar-wrap">
  <div class="toolbar">
    <div class="box-wrap">
      ...
      <div class="l-list">
        <ul class="icon-list">
          <li><a class="add" href="advert_group_edit.aspx?action=<%=QJEnums.ActionEnum.Add %>"><i></i><span>新增</span></a></li>
...
<ItemTemplate>
  <tr>
    ...
    <td><a href="advert_group_edit.aspx?action=<%=QJEnums.ActionEnum.Edit %>&id=<%#Eval("id")%>"><%#Eval("title")%></a></td>
    <td><%#GetTypeName(Eval("type").ToString())%></td>
    <td align="center"><%#Eval("view_num")%></td>
    <td><%#string.Format("{0:C}",Eval("price"))%>元/月</td>
    <td><%#Eval("view_width")%>×<%#Eval("view_height")%></td>
    <td><%#Eval("target")%></td>
    <td><%#string.Format("{0:g}",Eval("add_time"))%></td>
    <td align="center">
      <a href="advert_list.aspx?aid=<%#Eval("id")%>">内容</a>&nbsp;
      <a href="adv_view.aspx?id=<%#Eval("id")%>">调用</a>&nbsp;
      <a href="advert_group_edit.aspx?action=<%=QJEnums.ActionEnum.Edit %>&id=<%#Eval("id")%>">编辑</a>
    </td>
  </tr>
</ItemTemplate>
<FooterTemplate>
