$(function () {
    ...
    $.fn.InitUploader = function (b) {
        var fun = function (parentObj) {
            ...
            var btnView = $('<span class="upload-view" title="预览"></span>');//创建预览按钮

            //初始化单文件图片预览
            if (!p.multiple) {
                btnView.appendTo(parentObj);//将预览按钮插入到元素中
                btnView.click(function () {
                    viewImg($(this).attr("path"));
                });
                var imgSrc = parentObj.siblings(".upload-path").val();
                //当原路径中有图片地址时显示预览按钮
                if (imgSrc != "") {
                    setView(btnView, imgSrc);
                    //btnView.css({"width":"48px","height":"48px","top":"-8px", "display": "inline-block", "background-image": "url(" + imgSrc + ")", "background-position": "center center", "background-size": "cover" });
                    //btnView.attr("path", imgSrc);
                }
            }
            //初始化属性
            ...
            uploader.on('uploadSuccess', function (file, data) {
                ...
                if (data.status == '1') {
                    ...
                    if (!p.multiple) {
                        ...
                        setView(btnView, data.path);
                        //btnView.css({ "display": "inline-block" });
                        //btnView.attr("path", data.path);
                    } else {
    ...
    var d = parent.dialog({
        ...
        onshow: function () {
            var wait = setInterval(function () {
                var img = new Image();
                img.src = imgSrc;
                img.alt = "未能加载图片，请确保图片路径正确！";
                if (img.complete) {
                    clearInterval(wait);
                    var outResult = $('<div></div>');
                    var imgBox = $('<div class="upload-view-img"></div>').appendTo(outResult);
                    var _img = $(img).appendTo(imgBox);
                    d.content($(outResult).html());
                }
                else {
                    d.content('<div class="upload-view-img"><div class="waitting">正在加载图片</div></div>');
                }
            }, 200);
        },
        okValue: '关闭',
...
function setView(obj, imgSrc) {
    obj.css({ "width": "48px", "height": "48px", "top": "-8px", "display": "inline-block", "background-image": "url(" + imgSrc + ")", "background-position": "center center", "background-size": "cover" });
    obj.attr("path", imgSrc);
}