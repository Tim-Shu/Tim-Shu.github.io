...
//绑定需要浮动的表头
$(function () {
    $(".content-tab-ul-wrap li").eq(0).find("a").addClass("selected"); //默认第一个tab标签显示
    ...
    $(".rule-data-property").ruleDataProperty();
});
//全选取消按钮函数
...
//执行回传无复选框确认函数
function ExeLinkPostBack(obj, objmsg) {
    var msg = "删除记录后不可恢复，您确定吗？";
    var objID = $(obj).attr("href").replace(/'/gm, ".").split('.')[1];
    if (arguments.length == 2) {
        msg = objmsg;
    }
    $.dialog.confirm(msg, function () {
...
/*多项规格*/
$.fn.ruleDataProperty = function () {
    //获取参数 (.net后台调取数据库)
    var dataParam = $(this).attr("data");
    //创建添加，删除，确认按钮
    var propAdd = $("<a class=\"add-prop\" href=\"javascript:void(0);\" ><i></i><span>添加</span></a>").prependTo(this); //添加
    //创建UL列表
    var propUL = $("<ul id=\"field_prop_" + dataParam + "\"></ul>").prependTo(this);
    var dataObj = $("#field_control_" + dataParam);
    var dataArray = new Array();
    if (dataObj.length > 0) {
        dataArray = $("#field_control_" + dataParam).val().split(',');
        $.each(dataArray, function (key, val) {
            if (val.length > 0) {
                var propLI = $("<li></li>").appendTo(propUL);
                var propSpanName = $("<em>名称<em>").appendTo(propLI);
                var propInputName = $("<input type=\"text\" class=\"input\" value=\"" + val.split('|')[0] + "\" />").appendTo(propLI);
                var propSpanData = $("<em>参数<em>").appendTo(propLI);
                var propInputData = $("<input type=\"text\" class=\"input\" value=\"" + val.split('|')[1] + "\" />").appendTo(propLI);
                var propSaveBtn = $("<a class=\"save-prop\" href=\"javascript:void(0);\" ><i></i><span>确定</span></a>").appendTo(propLI); //确定
                var propDelBtn = $("<a class=\"del-prop\" href=\"javascript:void(0);\" ><i></i><span>删除</span></a>").appendTo(propLI); //删除                
                //绑定确认参数事件
                propSaveBtn.click(function (e) {
                    var dataIndex = $(this).parent().index();
                    var fieldDataName = $(this).prev().prev().prev();
                    var fieldDataValue = $(this).prev();
                    if ($(fieldDataName).val() == "") {
                        parent.jsprint("请填写规格名称", "", "Error");
                        $(this).prev().prev().prev().focus();
                        return false;
                    }
                    if ($(fieldDataValue).val() == "") {
                        parent.jsprint("请填写规格参数", "", "Error");
                        $(this).prev().focus();
                        return false;
                    }
                    dataArray.splice(dataIndex, 1, $(fieldDataName).val() + "|" + $(fieldDataValue).val());
                    $("#field_control_" + dataParam).val(dataArray.toString());
                    $(propAdd).show(); //显示添加按钮
                });
                //绑定删除事件
                propDelBtn.click(function (e) {
                    var dataIndex = $(this).parent();
                    var msg = "确认要删除该项规格吗？";
                    $.dialog.confirm(msg, function () {
                        dataArray.splice(dataIndex.index(), 1);
                        $("#field_control_" + dataParam).val(dataArray.toString());
                        $(dataIndex).remove();
                    });
                });
    ...
    //绑定添加事件
    propAdd.click(function (e) {
        //创建规格参数
        var propLI = $("<li></li>").appendTo(propUL);
        var propSpanName = $("<em>名称<em>").appendTo(propLI);
        var propInputName = $("<input type=\"text\" class=\"input\" />").appendTo(propLI);
        var propSpanData = $("<em>参数<em>").appendTo(propLI);
        var propInputData = $("<input type=\"text\" class=\"input\" />").appendTo(propLI);
        var propSaveBtn = $("<a class=\"save-prop\" href=\"javascript:void(0);\" ><i></i><span>确定</span></a>").appendTo(propLI); //确定
        var propDelBtn = $("<a class=\"del-prop\" href=\"javascript:void(0);\" ><i></i><span>删除</span></a>").appendTo(propLI); //删除                
        $(this).hide(); //隐藏添加按钮
        //绑定确认参数事件
